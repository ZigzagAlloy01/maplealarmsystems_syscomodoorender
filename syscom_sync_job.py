...
from syscom_sync_core import DEFAULT_MODELS, SyscomSyncCore


def cargar_modelos():
    modelos_env = os.getenv("SYSCOM_MODELS", "").strip()
    if modelos_env:
        texto = modelos_env.replace(",", "\n").replace(";", "\n")
        return SyscomSyncCore.parsear_modelos_texto(texto)

    modelos_file = os.getenv("SYSCOM_MODELS_FILE", "models_syscom.txt").strip() or "models_syscom.txt"
    if os.path.exists(modelos_file):
        with open(modelos_file, "r", encoding="utf-8") as f:
            return SyscomSyncCore.parsear_modelos_texto(f.read())

    return SyscomSyncCore.parsear_modelos_texto(DEFAULT_MODELS)
...
def main():
    try:
        ...
        sync = SyscomSyncCore(logger=print)
        print(f"Modelos a sincronizar: {len(modelos)}")

        resultado = sync.run_daily_sync(modelos)
        sync.guardar_modelos_no_encontrados("modelos_no_encontrados_syscom.txt")
        ...
        print(f"- Encontrados en SYSCOM: {carga['encontrados']}")
        print(f"- No encontrados en SYSCOM: {carga['no_encontrados']}")
        print(f"- Moneda mostrada: {carga['moneda_mostrada']}")
        ...
        print(f"Fallo la ejecucion automatica SYSCOM: {e}")
        traceback.print_exc()
        return 1
