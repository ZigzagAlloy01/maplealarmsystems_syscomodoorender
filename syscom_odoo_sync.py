...
from syscom_sync_core import DEFAULT_MODELS, DISPLAY_CURRENCY, SyscomSyncCore


class SyscomModelListApp(SyscomSyncCore):
    ...
    def actualizar_datos(self):
        ...
        try:
            ...
            messagebox.showerror("Error SYSCOM", f"No fue posible consultar SYSCOM:\n{e}")
            return

        self.repintar_tree()
        self.guardar_modelos_no_encontrados("modelos_no_encontrados_syscom.txt")
    ...
    def mostrar_detalle(self, event):
        ...
        cache_key = str(valores[0])
        producto = self.productos_cache.get(cache_key)
        if not producto:
            return
        ...
        texto_detalle = (
            ...
            f"Precio de compra SYSCOM: {self.obtener_precio_compra(producto.get('precios', {})):.2f} {DISPLAY_CURRENCY}\n"
            f"Precio de venta enviado a Odoo: {self.obtener_precio_venta(producto):.2f} {DISPLAY_CURRENCY}\n"
            f"Stock SYSCOM: {self.obtener_stock_producto(producto):.0f}\n"
            f"Peso: {self._formatear_numero(self.obtener_peso_producto(producto), 3)}\n"
            f"Volumen: {self._formatear_numero(self.obtener_volumen_producto(producto), 6)}\n\n"
            f"Descripcion:\n{self.obtener_nombre_producto(producto)}"
        )
    ...
    def _formatear_numero(self, valor, decimales):
        if valor is None:
            return "N/A"
